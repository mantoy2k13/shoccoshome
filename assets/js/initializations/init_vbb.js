...
var checkDateTime = (type)=>{
    ...
    if(date_from && date_to && time_start && time_end){
        ...
        if(given_date_from < date_today){
            ...
            if(type==1){ 
                var url = base_url + 'booking/book_user';
                var text = 'You have successfully book a user.';
            }else{
                var url = base_url + 'booking/update_book_user/'+$('#book_id').val();
